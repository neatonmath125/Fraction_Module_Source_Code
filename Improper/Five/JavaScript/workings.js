...
var Set2 = [["-",3,1,2],["",3,1,3],["-",3,2,3],["",3,1,4],["-",3,3,4],["",3,1,5],["-",3,2,5],["",3,3,5],["-",3,4,5],["",3,1,10],["",4,1,2],["-",4,1,3],["",4,2,3],["-",4,1,4],["",4,3,4],["-",4,1,5],["",4,2,5],["-",4,3,5],["",4,4,5],["-",4,1,10]]
...
function selector(id){
    localStorage.setItem("selector_choice",id)
    var check = document.getElementById(id)
    var new_id = id+"2"
    var newer5 = document.getElementById(new_id)
    var whole = parseInt(newer5.getAttribute("top"))
    var numerator = parseInt(check.getAttribute("top"))
    var denominator = parseInt(check.getAttribute("bottom"))
...
function checker(id){
    ...
    if(corral_denom != 0 && corral_denom != 0){
        if (((bottom * whole) + (whole/Math.abs(whole))*top) == numerator && bottom == denominator){
            ...
            answers_correct+=1
            setTimeout(red,200)
            setTimeout(green,400)
            setTimeout(red,600)
            setTimeout(green,800)
            setTimeout(red,1000)
            setTimeout(green,1200)
            setTimeout(red,1400)
            setTimeout(green,1600)
            setTimeout(red,1800)
            setTimeout(green,2000)
            setTimeout(red,2200)
            setTimeout(green,2400)
            setTimeout(red,2600)
            setTimeout(green,2800)
            setTimeout(red,3000)
            setTimeout(green,3200)
            setTimeout(red,3400)
            setTimeout(green,3600)
            setTimeout(red,3800)
            setTimeout(white,3950)
        }
    }
    if(answers_correct==5){
        answers_correct = 0
        document.getElementById("clock").className = "nothing11"
...
function red(){
    ...
    document.getElementById("bb835").style.color = "red"
}

function green(){
    document.getElementById("clock").style.backgroundColor = "green"
    document.getElementById("bb835").style.color = "green"
}

function white(){
    document.getElementById("clock").style.backgroundColor = "transparent"
    document.getElementById("bb835").style.color = "white"
}
...
function master(){
    if(turns < 3){
        ...
        for(var x = 0; x < correct_fractions.length; x++){
            correct_fractions[x].setAttribute('top',selected_answer[x][2])
            correct_fractions[x].setAttribute('bottom',selected_answer[x][3])
        ...
        for(var x = 0; x < correct_fractions2.length; x++){
            correct_fractions2[x].setAttribute('top',selected_answer[x][0] + selected_answer[x][1])
        ...
        for(var x = 0; x < choice_fractions.length; x++){
            choice_fractions[x].setAttribute('top',selected_answer[x][0] + (selected_answer[x][1] * selected_answer[x][3] + selected_answer[x][2]))
            choice_fractions[x].setAttribute('bottom',selected_answer[x][3])
            choice_fractions[x].style.opacity = 1
        }
        timout_explain = setTimeout(loader,60000)
        restarter_variable = setTimeout(restarter,63800)
        master_variable = setTimeout(master,64000)
    }
    else{
    var choice1 = document.getElementById("choice_one")
    var choice2 = document.getElementById("choice_two")
...
function very_easy(){
    ...
    localStorage.setItem("current_exercise","5049" );
    localStorage.setItem("next_exercise","5050" );
    localStorage.setItem("practice_title","Covert Operations C Workout 1");
...
function easy(){
    ...
    localStorage.setItem("current_exercise","5050" );
    localStorage.setItem("next_exercise","5051" );
    localStorage.setItem("practice_title","Covert Operations C Workout 2");
    document.location.href = "game.php"}
}
